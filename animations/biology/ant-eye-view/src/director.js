...
import { Vector3, Quaternion, Color3, Color4, MeshBuilder, Matrix, PBRMaterial, TransformNode } from '@babylonjs/core';
...
export function createDirector(ctx) {
  ...
  CH.olcek = (u, C, ch) => {
    ...
    if (u < C.blade) {
      ...
      c = cam(Vector3.Lerp(onG(9.5, 3.5, 2.2), base.add(V(-62, 30, -70)), k), Vector3.Lerp(f.pos.add(V(0, 1, 0)), Vector3.Lerp(base, top, 0.5), k), lerp(0.7, 0.8, k));
      applyCam(c);
  ...
  CH.koku = (u, C, ch) => {
    ...
    const f = stand(hero, -2, 9, 1.1, u, { headPitch: -0.2 + 0.08 * Math.sin(u * 1.3), ant: antennaMotion(u * 1.3, 3, tap) });
  ...
  CH.iz = (u, C, ch) => {
    ...
    if (smv > 0) {
      ...
      smell.set('trail', pf, camera, 0.12 * smv * strength * (1 - ramp(u, C.fade + 0.5, 4)));
    }
    const c = shots(u, [
      ...
      [C.recruit, uu => { const k = ease(clamp((uu - C.recruit) / 4)); return cam(V(lerp(10, 16, k), lerp(16, 300, k), lerp(-6, -30, k)), V(lerp(12, 18, k), 0, lerp(5, 7, k)), lerp(0.72, 0.2, k)); }, 1.6],
    ]);
    applyCam(c);
  ...
  CH.kisayol = (u, C, ch) => {
    ...
    if (!inLab) {
      ...
      applyCam(cam(V(16 + u * 0.3, 300, -30), V(18, 0, 7), 0.2));
  ...
  CH.kapanis = (u, C, ch) => {
    ...
    const c = shots(u, [
      ...
      [C.together, uu => cam(V(16 + (uu - C.together) * 0.4, 300, -30), V(18, 0, 7), 0.2), 1.4],
      [C.count - 1, uu => cam(V(16, lerp(300, 520, up), lerp(-30, -120, up)), V(lerp(18, 6, up), 0, lerp(7, 2, up)), lerp(0.2, 0.75, up)), 1.4],
    ]);
    applyCam(c);
  ...
  function update(t) {
    const { ch, u } = tl.at(t);
    const C = ch.cues;
    S = { macro: 2, focus: null, scale: 1, fade: 0, dist: 10 };
    hideAll();
    // reset shared state
